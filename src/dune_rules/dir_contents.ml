...
let expand_artifact a loc macro : Expander0.Expansion_result.t =
  Need_full_expander
    (fun expander ->
      ...
      let artifacts =
        ...
        let* artifacts =
          ...
          @@
          let open Memo.O in
          Super_context.find_exn ctx >>= get ~dir >>= artifacts
        in
        match a with
          ...
          (match Ml_sources.Artifacts.lookup_module artifacts name with
           ...
           | Some (t, m) ->
             (match Obj_dir.Module.cm_file t m ~kind:(Ocaml kind) with
              | None ->
                (* CR-rgrinberg: this seems odd *)
                Action_builder.return [ Value.String "" ]
              | Some path ->
...
let forms =
  Pform.Artifact.all
  |> Pform.Macro.Map.of_list_map_exn ~f:(fun artifact ->
    Pform.Macro.Artifact artifact, expand_artifact artifact)
;;

let () =
  let vars = Pform.Var.Map.empty in
  Expander0.Source.make vars forms
;;
